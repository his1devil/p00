...
$(function() {
  ...
  $(document).on('click', '.btn.btn-sm.ms', function() {
    resize(true);
    drawGraph();
  });
  ...
  $(document).on('click', '#docs-close', function() {
    deselectObject();
    return false;
  });
  //
...
function resize(showDocs) {
    ...
    graphHeight = window.innerHeight - docsHeight - 200;
    $graph.css('height', graphHeight + 'px');

    $close.css({
        bottom   : graphHeight + docsClosePadding + 'px',
        right : window.innerWidth - $docs.clientWidth + docsClosePadding + 'px'
    });
}
